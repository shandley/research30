...
Given a research topic, research30 queries all sources in parallel, scores results by relevance and academic signals, removes duplicates across sources, and outputs a ranked report. Results are output as a flat ranked list with abstract snippets, optimized for Claude to synthesize into research summaries. Typical queries complete in 2-5 seconds.
...
Default output (`--emit=compact`) is a flat ranked list sorted by score (0-100), showing the top 25 results. Each item includes score, title, source tag, date, URL, metadata, abstract snippet (first 200 chars), and relevance explanation. The abstract snippets allow Claude to synthesize findings across papers.

Display limits by depth:
- `--quick` — top 10
- default — top 25
- `--deep` — top 50

Full structured data is also written to `~/.local/share/research30/out/`:
- `report.json` -- full structured data (all results, not just top N)
...
- 50% keyword relevance (title match weighted 2x over abstract, bigram matching for multi-word queries, position boost for OpenAlex/S2 results)
...
research30/
  ...
  scripts/
    ...
    lib/
      ...
      render.py         -- Output formatting (flat ranked list, markdown, JSON)
